...
export async function getJobdeskByIdDb(jobdeskId){
    ...
    try {
        const[rows]= await connection.execute('SELECT * FROM jobdesk WHERE ID_Jobdesk = ?', [jobdeskId]);
        if (rows.length !== 0){
            return rows[0];
        } 
        else {
...
export async function getJobdeskByNameDb(nama_jobdesk){
    ...
    try {
        const [rows] = await connection.execute('SELECT * FROM jobdesk WHERE nama_jobdesk = ?', [nama_jobdesk]);
        if (rows.length !== 0){
            return rows[0];
        } 
        else {
