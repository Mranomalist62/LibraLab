...
const imagePaymentStorage = multer.diskStorage({
    // Define destination folder for uploaded files
    destination: function (req, file, cb) {
        cb(null, 'media/image/payment'); // 'uploads/' is the directory where the files will be stored
    },
...
// File size limit (e.g., 50MB) and file type validation (JPEG, PNG, GIF)
const imagePaymentUpload = multer({ 
    storage: imagePaymentStorage,
    limits: { fileSize: 50 * 1024 * 1024 },  // 50MB max file size
    fileFilter: function (req, file, cb) {
        const fileTypes = /jpeg|jpg|png|gif/;  // Accepted file types (images)
        const extname = fileTypes.test(path.extname(file.originalname).toLowerCase());
...
export { imageUpload, readableUpload, imagePaymentUpload, dynamicUpload };
