...
export async function postUserDb(userData) {
  let connection = await DbConnection();
  let queries =
    'INSERT INTO user(Nama_user, ID_Provinsi, ID_Kabupaten, Ket_alamat, notel_user, norek_user, password_user, email_user) VALUES (?,?,?,?,?,?,?,?)';
  try {
...
export async function putUserDb(userData, userId) {
  let connection = await DbConnection();
  let queries =
    'UPDATE user SET Nama_user = ?, ID_Provinsi = ?, ID_Kabupaten = ?, Ket_alamat = ?, notel_user = ?, norek_user = ?, password_user = ?, email_user = ? WHERE ID_User = ?;';
  try {
    const rows = await connection.execute(queries, [
      userData.Nama_user,
      userData.ID_Provinsi,
...
export async function SignUpUserDb(userData) {
  let connection = await DbConnection();
  let queries =
    'INSERT INTO user(Nama_user, password_user, email_user) VALUES (?,?,?)';
  try {
    const rows = await connection.execute(queries, [
      userData.Nama_user,
      userData.password_user,
