  ...
  try {
    if (await authorValidation.isEmailAvailable(authorData)) {
      if (await authorValidation.isDataSignUpExist(authorData)) {
        let otp_status = await OTPMiddleware.sendOTPEmailVerification(
...
export async function finishSignUp(req, res) {
  ...
  try {
    ...
    if (await authorValidation.isDataOTPExist(authorData)) {
      ...
      if (result === 200) {
        authorData.password_author = await hashingMiddleware.hashPassword(
        ...
        if (SignUpStatus === 503 || SignUpStatus === null) {
          ...
          res
            .status(200)
            .json({ message: 'OTP confirmed, sign up success', status: 200 });
        }
      } else if (result === 404) {
        res
          .status(404)
          .json({ message: 'no OTP found for said address, sign up failed' });
      } else if (result === 400) {
        res
          .status(400)
          .json({ message: 'otp is expired, initiate sign up again' });
      } else {
        res
          .status(500)
          .json({ message: 'Error verifying OTP,  sign up failed' });
      }
    } else {
      res.status(400).json({ message: 'required information is missing' });
    }
...
export async function loginAuthor(req, res) {
  ...
  try {
    if (await authorValidation.isDataLoginExist(authorData)) {
      const result = await authorModel.getAuthorByEmailDb(
        authorData.email_author
      );
      if (result !== null) {
        if (
          await hashingMiddleware.isHashedPasswordMatch(
            authorData.password_author,
            result.password_author
          )
        ) {
          ...
          if (token === 500) {
            ...
            res
              .status(200)
              .json({ message: 'Credential confirmed, Login success' });
          }
        } else {
